        ...
        if(flag>0){
            ...
            if(manyClassflag==0){
                if (originalPositiveLabel[max]==testLabel[i])
                    correctIns++;
        if(i<10){
            cout<<"originPoslabel "<<originalPositiveLabel[max]<<endl;
            cout<<"real label "<<testLabel[i]<<endl;
            cout<<"decvalue "<<combDecValue[1][i]<<endl;
            }}
        }
        else{
            noClassIns++;
...
void trainOVASVM(SVMParam &param, string strTrainingFileName, int numFeature,  bool evaluteTrainingError, string strTestingFileName, ofstream &ofs) {
    ...
    LibSVMDataReader drHelper;
    drHelper.ReadLibSVMAsSparse(v_v_Instance, v_nLabel, strTrainingFileName, numFeature);
/*		for(int te=0;te<v_nLabel.size();te++){
			if (v_nLabel[te]>0)
				v_nLabel[te]=-1;
			else
				v_nLabel[te]=1;
    ...
    drHelper2.ReadLibSVMAsSparse(testInstance, testLabel, strTestingFileName, numFeature);
	...
	cout<<"origin "<<originalPositiveLabel[0]<<" "<<originalPositiveLabel[1];    
...
*/
    ...
    if(i>0){
        ...
        int tempLabel;
		cout<<"instance "<<v_v_Instance[0][2].id<<endl;
		cout<<"instance "<<v_v_Instance[2][2].id<<endl;
    //swap label
    ...
    //swap instance 
		...
		cout<<"after swap instance "<<v_v_Instance[0][2].id<<endl;
		cout<<"after swap instance "<<v_v_Instance[2][2].id<<endl;
